...
private[spark] class ExecutorAllocationManager(
  ...
  private def removeExecutors(executors: Seq[String]): Seq[String] = synchronized {
    ...
    val (execsToBeRemoved, execsToLeave) = executors.splitAt(numExistingExecutors - minNumExecutors)
    execsToLeave.foreach { execId =>
      logDebug(s"Not removing idle executor $execId because there are only " +
